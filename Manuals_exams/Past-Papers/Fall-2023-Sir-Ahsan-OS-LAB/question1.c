...
int main() {
    ...
    if (pv1_pid == 0) {
        ...
        for (int i = 0; i < NUM_FILES; i++) {
            write(pipe_PV1_PV2[1], fileNames[i], strlen(fileNames[i]) + 1);
        }
    ...
    if (pv2_pid == 0) {
        ...
        ssize_t n;
        while ((n = read(pipe_PV1_PV2[0], filename, MAX_FILENAME_LEN)) > 0) {
            // Transcode simulation: append "_transcoded"
            strcat(filename, "_transcoded");
            write(pipe_PV2_PV3[1], filename, strlen(filename) + 1);
        }
    ...
    if (pv3_pid == 0) {
        ...
        char filename[MAX_FILENAME_LEN];
        ssize_t n;
        while ((n = read(pipe_PV2_PV3[0], filename, MAX_FILENAME_LEN)) > 0) {
            // QC simulation: append "_QC_Passed"
            strcat(filename, "_QC_Passed");
            write(pipe_PV3_PV4[1], filename, strlen(filename) + 1);
        }
    ...
    if (pv4_pid == 0) {
        ...
        char filename[MAX_FILENAME_LEN];
        ssize_t n;
        while ((n = read(pipe_PV3_PV4[0], filename, MAX_FILENAME_LEN)) > 0) {
            // Distribution simulation: print or store the file
